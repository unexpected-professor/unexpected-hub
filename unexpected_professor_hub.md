...
| UPH-003 | Identity | Register domain and enable registrar MFA/recovery | NOT STARTED | UPH-002 | Registrar chosen: OVHcloud (ADR-018). Owner to register `theunexpectedprofessor.com` (+ `unexpectedprofessor.com` redirect), enable account 2FA and registrar-lock | 2026-09-01 |
...
| UPH-015 | Site | Implement canonical lesson template and navigation between lessons | DONE | UPH-013 | `apps/site/src/pages/lessons/[...id].astro`: title/summary/level/duration, consent-gated video, objectives, prerequisites, body, lab link, sources/credits, author+licence+dates, and previous/next navigation computed from `course_sequence` + `sequence_index`. Commit 3 | 2026-09-01 |
...
| UPH-036 | Site polish | Per-sequence sidebar, "suggest an edit on GitHub" + "last updated" per lesson, Pagefind static search, subject landing pages, footer attribution/funder block | DEFERRED | UPH-031 | Structural ideas from `learn.libre.solar`; see `documentation/why-astro.md` | 2026-09-02 |
...
| ADR-004 | 2026-09-01 | Use Astro with Markdown/MDX for the public educational site | ACCEPTED | Islands architecture ships zero JS by default, bespoke components (consent-gated video, lab embed) are first-class, and content collections validate lesson front matter at build. Full rationale and the alternatives considered (Quarto, Docusaurus, VitePress, mdBook, ...) in `documentation/why-astro.md` |
...
| ADR-018 | 2026-09-01 | Use OVHcloud for both the domain registrar and the pilot VPS, on one account | ACCEPTED | French/EU provider matching the project's legal posture; free WHOIS privacy; domain, DNS, and VPS-1 (2 vCore / 4 GB / 40 GB NVMe, ~EUR 4.6/month TTC, daily backup) on one invoice. OVH shared web hosting is explicitly rejected — it cannot run the Dash container |
...
| 2026-09-01 | Lab responsive layout | The Dash three-pane layout was unusable in the ~1150 px embed. Fixes: the lesson lab section now breaks out of the 68ch reading column to the full content width; `assets/responsive.css` + `three-pane*` class names stack the panes vertically below 992 px (overriding the inline flex styles); graph columns switch from fixed half-width to `xs=12 xxl=6` so they stack until 1400 px | 39 pytest still pass; `responsive.css` served and linked; both apps build | `fix(labs): stack the three-pane layout on narrow viewports` | — |
| 2026-09-02 | Astro rationale + site backlog | Wrote `documentation/why-astro.md` (expands ADR-004 with the archetype framing, a 10-tool comparison, and what Astro does not give for free); reviewed `learn.libre.solar` (VuePress 1.x, EOL) for structural ideas | ADR-004 → ACCEPTED; new backlog row UPH-036 | `docs: record the Astro rationale and site-structure backlog` | Phase 2 provisioning unchanged |
